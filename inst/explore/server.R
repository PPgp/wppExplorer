 ...
 # }, include.rownames = FALSE, include.colnames = FALSE)
  	...
  	if(!is.null(low)) {
  		...
  		for(i in 3:1) {
  			...
  			g <- g + geom_ribbon(aes(ymin=.data[[colnames(data)[idx][1]]], ymax=.data[[colnames(data)[idx][2]]], 
  											fill=.data[["charcode"]], colour=.data[["charcode"]], linetype="blank"), 
  											alpha=c(0.3, 0.2, 0.1)[i])
  ...
  .print.pyramid <- function(data) {
  	...
  	if(is.element('low', colnames(data))) {
  		g <- g + geom_ribbon(data=subset(data, sex=='F'), aes(ymin=low, ymax=high, linetype='blank'), alpha=0.3)
  		g <- g + geom_ribbon(data=subset(data, sex=='M'), aes(ymin=high, ymax=low, linetype='blank'), alpha=0.3)
