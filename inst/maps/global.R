...
lookupByIndicator <- function(indicator) {
	...
	do.call('data', list(data.env$datasets.obs[indicator], package='wpp2012', envir=data.env))
  	...
  	if(!is.na(data.env$datasets.obs[indicator])){
  		...
  		# load predictions
  		do.call('data', list(data.env$datasets.pred[indicator], package='wpp2012', envir=data.env))
...
getUncertainty <- function(indicator, what='low') {
	...
	do.call('data', list(data.env[[datasetname]][indicator], package='wpp2012', envir=data.env))
